    ...
    func loadComments(feedId: String) async {
        do {
            ...
            let persistComments: [Comment] = try await supabase
                .from("comments")
                .select("id, feed_id, user_id, parent_comment_id, content, status, created_at, updated_at, is_edited, is_hidden, hidden_by, profiles!user_id(id, nickname, avatar_url, is_official, deleted_at)")
